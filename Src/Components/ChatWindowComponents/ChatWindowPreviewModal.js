import { StyleSheet, Text, View, TouchableOpacity, Image, TextInput, ToastAndroid, Pressable, ActivityIndicator } from "react-native";
...
import { setMediaData, setAsPremium } from "../../../Redux/Slices/NormalSlices/MessageSlices/ChatWindowPreviewDataSlice";
...
import { generateVideoThumbnail, videoReducer } from "../../../FFMPeg/FFMPegModule";
...
import { ChatWindowError, ChatWindowFollowError, LoginPageErrors, OfflineSnack } from "../ErrorSnacks";
...
const ChatWindowPreviewModal = ({ chatRoomId, name }) => {
  ...
  const previewModalShow = useSelector((state) => state.hideShow.visibility.chatWindowAttachmentPreviewModal);
  ...
  const [followUser, { isError: followError, data: followData }] = useFollowUserMutation();
          ...
          if (e?.data?.statusCode === 200) {
            ...
            sendMessage({
              ...
              .then((e) => {
                ...
                dispatcher(updateCacheRoomList({ chatRoomId, createdAt: e?.data?.data?.createdAt, message: "", hasAttachment: true, senderId: e?.data?.data?.sender?._id }));
                setAmount(undefined);
                setDisableSendButton(false);
                dispatcher(
                  ...
                  uploadAttachment({ token, formData }).then((e) => {
                    ...
                    if (e?.data?.statusCode === 200) {
                      ...
                      sendMessage({
                        ...
                        .then((e) => {
                          console.log(green("Video Attachemet Sent with After message"));
                          dismissProgressNotification();
                          dispatcher(updateCacheRoomList({ chatRoomId, createdAt: e?.data?.data?.createdAt, message: "", hasAttachment: true, senderId: e?.data?.data?.sender?._id }));
                          setAmount(undefined);
                          setDisableSendButton(false);
  ...
  return (
    ...
    >
      <View style={styles.modalInnerWrapper}>
        <DIcon provider={"Entypo"} name={"circle-with-cross"} size={responsiveWidth(6)} color={"#ffa07a"} style={{ alignSelf: "flex-end" }} onPress={() => handlePreviewModalClose()} />
        <Text style={styles.attachFileText}>Attach File</Text>
        <View style={styles.previewModalImageWrapper}>
          {attachmentType !== "video" ? (
            <Image source={!mediaPath ? require("../../../Assets/Images/Profile.jpg") : { uri: mediaPath }} style={{ flex: 1, width: "100%", resizeMode: "cover", borderRadius: responsiveWidth(4) }} resizeMethod="resize" resizeMode="cover" />
          ) : (
            <VideoPlayer
              ref={videoRef}
              style={{
        ...
        <View style={[styles.previewModalInputWrapper]}>
          <TextInput editable={!next} multiline style={[styles.previewModalInput, next ? { borderColor: "#FFA07A", borderWidth: 2 } : { borderColor: "#282828" }]} textAlignVertical="top" placeholder="Attachment text" placeholderTextColor={"gray"} ref={attachmentInputRef} onChangeText={(t) => (attachmentInputRef.current.value = t)} />
        </View>
        <View style={styles.modalSendContainer}>
          <View style={styles.freePaidToggle}>
            {/* <Text style={styles.freePaidToggleText}>{isAttachmentPremium ? "Paid" : "Free"}</Text>
            <Switch trackColor={{ false: "#e7b5f6", true: "#81b0ff" }} thumbColor={isAttachmentPremium ? "#f5dd4b" : "purple"} onValueChange={() => handleAttachmentAsPremium()} value={isAttachmentPremium} /> */}

            <TouchableOpacity style={[styles.free, isAttachmentPremium === false ? { backgroundColor: "#a7dbd8" } : null]} onPress={() => handleAttachmentAsPremium(false)}>
              <Text style={{ fontFamily: "MabryPro-Regular", fontSize: responsiveFontSize(2.3), color: "#282828" }} key={"1Free"}>
                Free
              </Text>
            </TouchableOpacity>

            <TouchableOpacity style={[styles.paid, isAttachmentPremium === true ? { backgroundColor: "#a7dbd8" } : null]} onPress={() => handleAttachmentAsPremium(true)}>
              <Text key={"2Paid"} style={{ fontFamily: "MabryPro-Regular", fontSize: responsiveFontSize(2.3), color: "#282828" }}>
                Paid
              </Text>
            </TouchableOpacity>
          </View>
          ...
          {isAttachmentPremium ? (
            <View style={styles.amountInput}>
              ...
              <TextInput maxLength={5} editable={!next} placeholder="Coins 0" keyboardType="number-pad" style={{ borderBottomWidth: 1, padding: 0, paddingLeft: 2 }} value={amount} onChangeText={(t) => setAmount(t.replace(/[^0-9]/g, ""))} />
          ...
          {!next ? (
            ...
            <View style={styles.confirmationView}>
              <Text style={styles.confirmationViewTextTitle}>You are about to send attachment ?</Text>
